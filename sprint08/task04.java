...
//
//Define the type for goShopping field and name it DecisionMethod and define a method decide
// in it.
//
//Also, we have the class Shop with method sale. This method informs users about a discount
// product and a percentage of discount by using their goShopping values
// (which are stored in clients field). The method should return the count of users that
// will go shopping.
...
class Shop{
    public List<DecisionMethod> clients = new ArrayList<DecisionMethod>();

    public int sale(String product, int percent){
        int count = 0;
        for(int i = 0; i < clients.size(); i++){
            if(clients.get(i).decide(product, percent)) count++;
        }
        return count;
    }
}
